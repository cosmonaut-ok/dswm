...
(defclass tray ()
  ((win :initform nil :initarg :win :accessor tray-win) ;; Main window
   (sowin :initform nil :initarg :sowin :accessor tray-sowin) ;; Selection Owner Window (required by the tray protocol)
   (fpwin :initform nil :initarg :fpwin :accessor tray-fpwin) ;; Focus proxy window (required by the xembed protocol)
   (viwin :initform nil :initarg :viwin :accessor tray-viwin) ;; Visible Icons container
   (hiwin :initform nil :initarg :hiwin :accessor tray-hiwin) ;; Hidden icons container
   (curwin :initform nil :initarg :curwin :accessor tray-curwin) ;; Icon cursor window
   (curpos :initform nil :initarg :curpos :accessor tray-curpos) ;; Icon cursor position (icon index)
   (icon-height :initform nil :initarg :icon-height :accessor tray-icon-height) ;; Icon height
   ;; Whether hidden icons are visible or not
   (show-hiwin-p :initform nil :initarg :show-hiwin-p :accessor tray-show-hiwin-p)
   (vicons :initform nil :initarg :vicons :accessor tray-vicons) ;; visible icon list
   (hicons :initform nil :initarg :hicons :accessor tray-hicons) ;; hidden icon list
   ;; Visible icons orderin information
   (vicons-ordering :initform nil :initarg :vicons-ordering :accessor tray-vicons-ordering)
   ;; Hidden icons ordering information
   (hicons-ordering :initform nil :initarg :hicons-ordering :accessor tray-hicons-ordering)
   ;; Hook function added to *event-processing-hook* to process tray events
   (event-processing-fn :initform nil :initarg :event-processing-fn :accessor tray-event-processing-fn)
   (display :initform nil :initarg :display :accessor tray-display)
   (screen :initform nil :initarg :screen :accessor tray-screen)
   (mode-line :initform nil :initarg :mode-line :accessor tray-mode-line)))

;;; generic functions
(defgeneric screen-mode-line (screen)
  (:documentation ""))
(defgeneric trayicon-height (tray)
  (:documentation ""))
(defgeneric screen-tray-height (screen)
  (:documentation ""))
(defgeneric screen-tray-icon-height (screen)
  (:documentation ""))
(defgeneric tray-height (tray)
  (:documentation ""))
(defgeneric tray-width (tray)
  (:documentation ""))
(defgeneric tray-position-right (tray)
  (:documentation ""))
(defgeneric destroy-tray (tray)
  (:documentation ""))
(defgeneric map-tray (tray)
  (:documentation ""))
;; (setf tray-icons) (value tray visibility) ;; TODO: make defgeneric
;; (setf tray-ordering) (value tray visibility) ;; TODO: make defgeneric
(defgeneric icon-at-cursor (tray)
  (:documentation ""))
(defgeneric tray-tile-icons (tray)
  (:documentation ""))
(defgeneric tray-sort-icons (tray)
  (:documentation ""))
(defgeneric update-icon-containers-geometry (tray)
  (:documentation ""))
(defgeneric update-cursor-geometry (tray)
  (:documentation ""))
(defgeneric update-main-window-geometry (tray)
  (:documentation ""))
(defgeneric tray-update-geometry (tray)
  (:documentation ""))
(defgeneric show-hiwin (tray)
  (:documentation ""))
(defgeneric hide-hiwin (tray)
  (:documentation ""))
(defgeneric move-icon-left (tray)
  (:documentation ""))
(defgeneric move-icon-right (tray)
  (:documentation ""))
(defgeneric xembed-tray-init (tray)
  (:documentation ""))
(defgeneric fdo-tray-selection-name (tray)
  (:documentation ""))
(defgeneric fdo-tray-init-properties (tray)
  (:documentation ""))
(defgeneric fdo-tray-send-manager-notification (tray)
  (:documentation ""))
(defgeneric fdo-tray-init (tray)
  (:documentation ""))
(defgeneric fdo-tray-make-event-handler (tray)
  (:documentation ""))
(defgeneric make-tray-xembed-event-handler (tray)
  (:documentation ""))
(defgeneric make-tray-handler (tray)
  (:documentation ""))
(defgeneric tray-init (tray)
  (:documentation ""))
(defgeneric tray-window-list (tray)
  (:documentation ""))
(defgeneric tray-socket-list (tray)
  (:documentation ""))
(defgeneric tray-client-list (tray)
  (:documentation ""))
...
(defmethod screen-mode-line ((screen screen)) ;; TODO: incorrect name
...
(defmethod tray-height ((tray tray))
  "Returns the TRAY height."
  (screen-tray-height (tray-screen tray)))

(defmethod tray-width ((tray tray))
...
(defmethod tray-position-right ((tray tray))
...
;; Colors
(defparameter *tray-win-background* (nth 7 *colors*)
  "Tray main container window background color.")

(defparameter *tray-viwin-background* *mode-line-background-color*
  "Tray visible icons container window color.")

(defparameter *tray-hiwin-background* *mode-line-border-color*
  "Tray hidden icons container window color.")

(defparameter *tray-cursor-color* (nth 2 *colors*)
...
(defun create-tray (screen mode-line &key (x 0) (y 0))
  ...
  (let* ((parent (mode-line-window mode-line))
	 ...
	 (win (xlib:create-window :parent parent
                                  ...
                                  :background (xlib:alloc-color (xlib:window-colormap root-window)
                                                               (lookup-color
                                                                (current-screen) *tray-win-background*))
    ...
    (flet ((create-1x1-invisible-window (event-mask)
      ...
      (let* ((fpwin (create-1x1-invisible-window +FPWIN-EVENT-MASK+))
        ...
        (make-instance 'tray :win win :sowin sowin :fpwin fpwin
...
(defmethod destroy-tray ((tray tray))
  "Destroys and de-initializes a tray object."
  (remove-hook *event-processing-hook* (tray-event-processing-fn tray))
...
(defmethod icon-at-cursor ((tray tray))
...
(defmethod tray-tile-icons ((tray tray))
...
(defmethod tray-sort-icons ((tray tray))
...
(defmethod update-icon-containers-geometry ((tray tray))
...
(defmethod update-cursor-geometry ((tray tray))
...
(defmethod update-main-window-geometry ((tray tray))
...
(defmethod tray-update-geometry ((tray tray))
...
(defmethod show-hiwin ((tray tray))
  "Shows the hidden icons in TRAY."
  (set-hiwin-visibility tray t))

(defmethod hide-hiwin ((tray tray))
...
(defmethod move-icon-left ((tray tray))
...
(defmethod move-icon-right ((tray tray))
...

;;;; Xembed requirements
(defmethod xembed-tray-init ((tray tray))
...
(defmethod fdo-tray-send-manager-notification ((tray tray))
...
(defmethod fdo-tray-init ((tray tray))
...
(defmethod fdo-tray-make-event-handler ((tray tray))
...
(defmethod make-tray-xembed-event-handler ((tray tray))
...
(defmethod make-tray-handler ((tray tray))
...
(defmethod tray-socket-list ((tray tray))
...
(defun new-mode-line-hook (mode-line)
  ...
  (let ((dswm-screen (mode-line-screen mode-line)))
    (unless (screen-tray dswm-screen)
      (let* ((tray (create-tray dswm-screen mode-line))
        ...
        (let ((event-handler (lambda ()
                               (loop while (ignore-errors
                                             (xlib:process-event *display* :timeout 0 :handler hnd))))))
...
(defun destroy-mode-line-hook (mode-line)
  ...
  (let* ((dswm-screen (mode-line-screen mode-line))
    ...
    (when (and
           tray
           (xlib:window-equal
            (mode-line-window (tray-mode-line tray))
            (mode-line-window mode-line)))
      (destroy-tray tray))))
...
(defcommand systray () ()
  ...
  (if (current-tray)
      ...
      (let* ((dswm-screen (current-screen))
             (tray (create-tray dswm-screen (screen-mode-line dswm-screen)))
             (hnd (make-tray-handler tray)))
        (setf (screen-tray dswm-screen) tray)
        (tray-init tray)
        (map-tray tray)
        (let ((event-handler (lambda ()
                               (loop while (ignore-errors
                                             (xlib:process-event *display* :timeout 0 :handler hnd))))))
          (setf (tray-event-processing-fn tray) event-handler)
          (add-hook *event-processing-hook*
                            event-handler)))))
